...
def to_grid_mipmap(x):
    """
    this changes a UV map with mipmap levels in B channel as tensor image (CHW)
    to a grid valid for usage with grid_sample while keeping the mipmap levels in B channel unchanged
    """
    uv = to_grid_range(x[:2])
    lod = x[2].unsqueeze(0)
    x = torch.cat((uv, lod), dim=0)
    x = chw_to_hwc(x)
    return x


def from_grid_mipmap(x):
    """
    this changes a grid valid for usage with grid_sample with mipmap levels in B channel
    to a UV map as tensor image (CHW) while keeping the mipmap levels in B channel unchanged
    """
    x = hwc_to_chw(x)
    uv = from_grid_range(x[:2])
    lod = x[2].unsqueeze(0)
    x = torch.cat((uv, lod), dim=0)
    return x
...
def get_uv_mipmap_transform():
    return Compose([
        ToTensor(),
        Lambda(to_grid_mipmap)
    ])